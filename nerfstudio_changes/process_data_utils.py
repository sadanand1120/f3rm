...
def convert_video_to_images(
    ...
    # Check if images already exist for all required scales
    all_images_exist = True
    expected_frame_count = None

    for i in range(num_downscales + 1):
        scale_dir = image_dir if i == 0 else Path(f"{image_dir}_{2**i}")
        if not scale_dir.exists():
            all_images_exist = False
            break

        # Check if we have the expected number of images
        existing_images = list(scale_dir.glob(f"{image_prefix}*.png"))
        if expected_frame_count is None:
            expected_frame_count = len(existing_images)
        elif len(existing_images) != expected_frame_count:
            all_images_exist = False
            break

    if all_images_exist and expected_frame_count > 0:
        CONSOLE.log(f"[bold green]Images already exist ({expected_frame_count} frames across {num_downscales + 1} scales), skipping video conversion.")
        summary_log = []
        summary_log.append(f"Found {expected_frame_count} existing images with prefix '{image_prefix}'")
        return summary_log, expected_frame_count
