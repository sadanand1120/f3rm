...
f3rm_method = MethodSpecification(
    config=F3RMTrainerConfig(
        ...
        steps_per_eval_image=5751,   # Match baseline eval-image residue while staying near run end.
        steps_per_eval_all_images=5753,  # Keep one near-end eval-all pass near the final iterations.
        save_only_latest_checkpoint=True,
        steps_per_save=3700,
        max_num_iterations=5760,
        ...
        pipeline=FeaturePipelineConfig(
            datamanager=FeatureDataManagerConfig(
                ...
                train_num_rays_per_batch=11_520,
                train_num_images_to_sample_from=32,
                train_num_times_to_repeat_images=360,
        ...
        optimizers={
            "proposal_networks": {
                ...
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-4, warmup_steps=720, max_steps=5760),
            },
            "fields": {
                "optimizer": AdamOptimizerConfig(lr=1e-2, eps=1e-15, max_norm=1.0),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-4, warmup_steps=720, max_steps=5760),
            ...
            "feature_field": {
                ...
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=5e-3, warmup_steps=720, max_steps=5760),
            },
            "camera_opt": {
                "optimizer": AdamOptimizerConfig(lr=1e-4, eps=1e-8, weight_decay=0.0, max_norm=0.5),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-5, warmup_steps=2160, max_steps=5760),
            },
        },
