...
from f3rm.features.orientany2.vggt.models.vggt import VGGT
from f3rm.features.orientany2.vggt.layers import Mlp
from f3rm.features.orientany2.vggt.layers.block import Block
...
def load_patch_embed_weights(model, checkpoint_path):
    ...
    # 3. Filter parameters that only contain aggregator.patch_embed
    patch_embed_state = {
...
class VGGT_OriAny_Ref(nn.Module):
    ...
    def forward(self, img_inputs):
        ...
        with torch.amp.autocast(device_type='cuda', dtype=self.dtype):
            ...
            if S>1:
                # Separate the first token of each batch and the remaining tokens
                ref_tokens = pose_tokens[:, 0, :]            # shape: (B, C)
                tgt_tokens = pose_tokens[:, 1:, :]           # shape: (B, S-1, C)

                # Downsample
                ref_feat = self.ref_sampler(ref_tokens)      # shape: (B, C'), assuming output channel is C'
