...
class OrientAny:
    def __init__(self, ckpt_dir='f3rm/features/orientany/ckpts', model_name='ronormsigma1_dino_weight.pt', device=None):
    ...
    @staticmethod
    def project_axes_to_image(R_objw2cam, radius, axes_len, img_width, img_height):
        ...
        K = OrientAny.get_K(r=0.5, t=0.5, n=3.0, img_w=img_width, img_h=img_height)
        pcs_pts, _ = Homography.projectCCStoPCS(ccs_pts, K, img_width, img_height)
        return pcs_pts
    ...
    def draw_axes_on_image(image, R_objw2cam, radius=16, axes_len=2):
        ...
        if pcs_pts is None:
            print(f"Warning: No valid projected points.")
            return image, None
        T_objw2cam = OrientAny.get_T_from_R(R_objw2cam)
        T5 = Homography.get_std_trans(cz=-radius)
        T_wcs_to_ccs = T5 @ T_objw2cam
    ...
    def preprocess_remove_bkg(input_image, do_remove_background):
        if do_remove_background:
            ...
            # if not RGBA -> user has not provided a background alpha mask -> just return the image
